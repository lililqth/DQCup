...
public class Vote {
	...
	public HashSet<RepairedCell> repair() {
		...
		for (Tuple tuple : tuples) {
			if (!tuple.getValue("CUID").equals(currentCUID)) {
				// 投票
				if (record != null) {
					for (int i = 0; i < 9; i++) {
...
//										RepairedCell cell = new RepairedCell(RUID, name, record.maxKey[i]);
//										result.add(cell);
				...
				// 刷新
				record = new Record();
				currentCUID = tuple.getValue("CUID");
			}
			
			// 建立value->RUID_List对，对每一个tuple的10个属性进行处理  不处理zipcode
			for (int i = 2; i < 11; i++) {
				String value = tuple.getValue(this.itemNameList[i]);
