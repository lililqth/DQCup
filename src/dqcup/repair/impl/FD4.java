...
public class FD4 {
    ...
    public FD4(LinkedList<Tuple> tup) {
        this.tuples = tup;
    }

    public HashSet<RepairedCell> repair() {
    	// 和 FD1 的X一样，因此不需要重新排序
//        Collections.sort(tuples, new TupleCompare_STADD_CITY()); // 排序
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (val.size() < record.maxLength || key.equals("null")) {
                            for (String str : val) {
                                ...
                                if(personLastRecord.set("STNUM", record.maxKey)){
                                	result.add(new RepairedCell(RUID, "STNUM", record.maxKey));
                                }

                                // 将这个人的每一条记录全部修改过来
                                for (int i=1; i<= personLastRecord.number; i++){
                                    result.add(new RepairedCell(RUID-i, "STNUM", record.maxKey));
                                }
                            }
                        }
                    }
    ...
    private void addRecord(Tuple current, RecordXY record) {
        // 建立value->RUID_List对
        String value = current.getValue("STNUM");
